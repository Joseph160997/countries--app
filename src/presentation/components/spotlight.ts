...
export const renderSpotlight = (country: Country): string => {
  ...
  return `
  <section class="mb-12 overflow-hidden rounded-2xl border border-slate-200/80 dark:border-starlight/10 bg-white dark:bg-space-deep/60 shadow-xl shadow-slate-200/40 dark:shadow-none animate-fade-in-up">
    <div class="flex flex-col md:flex-row">
      <!-- Bandera -->
      <div class="relative md:w-2/5 h-56 md:h-auto overflow-hidden bg-slate-100 dark:bg-slate-900/40">
        <img src="${country.flag}" alt="Flag of ${country.name}" loading="lazy" class="w-full h-full object-cover"/>
        <span class="absolute top-4 left-4 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] bg-ink/85 text-starlight backdrop-blur-sm">
          Country of the Day
        </span>
      </div>
      ...
      <div class="md:w-3/5 p-6 lg:p-10 flex flex-col justify-center">
        <p class="font-mono text-xs font-semibold uppercase tracking-[0.25em] text-emerald-600 dark:text-emerald-400 mb-3">${today}</p>
        ...
        <div class="flex flex-wrap items-center gap-x-8 gap-y-4 mb-8">
          ...
          <div class="flex flex-col">
            <span class="text-[10px] font-bold uppercase tracking-wider text-ink-soft/70 dark:text-slate-500">Capital</span>
            <span class="font-mono text-sm font-semibold text-ink dark:text-starlight">${country.capital}</span>
          </div>
          <div class="flex flex-col">
            <span class="text-[10px] font-bold uppercase tracking-wider text-ink-soft/70 dark:text-slate-500">Population</span>
        ...
        <div>
          <button
            ...
            class="btn-spotlight-explore inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm uppercase tracking-wider transition-all cursor-pointer shadow-md active:scale-95"
          >
