...
export const renderWeatherWidget = (
  ...
  if (status === "loading") {
    ...
    content = `
      <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/30 text-xs text-slate-400 dark:text-slate-500 italic">
        Weather unavailable for ${capital}
      </div>`;
  } else {
    content = `
      <div class="p-4 rounded-xl bg-linear-to-r from-sky-50 to-blue-50 dark:from-sky-950/40 dark:to-blue-950/40 border border-sky-100 dark:border-sky-900/40">
